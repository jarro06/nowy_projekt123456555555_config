...
def job_config = [
        job: [
                name: "nowy_projekt123456555555_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'nowy_projekt123456555555_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/nowy_projekt123456555555_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.nowy_projekt123456555555.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "nowy_projekt123456555555_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
